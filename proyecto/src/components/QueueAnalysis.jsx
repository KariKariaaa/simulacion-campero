...
export default function QueueAnalysis({ data, onAnalysisParams }) {
  ...
  try {
    const statistics = calculateQueueStatistics(data);
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1">
            <label className="block text-sm font-semibold text-gray-700 mb-3">
              Tiempo máximo de espera aceptable (HH:MM:SS)
            </label>
            <input
              type="text"
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottomColor: '#fbd816', borderBottomWidth: '2px' }}>
                <th className="px-4 py-3 text-left font-semibold text-gray-700">Día</th>
                <th className="px-4 py-3 text-center font-semibold text-gray-700">Clientes</th>
                <th className="px-4 py-3 text-center font-semibold text-gray-700">Prom. Espera</th>
                <th className="px-4 py-3 text-center font-semibold text-gray-700">Máx. Espera</th>
                <th className="px-4 py-3 text-center font-semibold text-gray-700">Mín. Espera</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(statistics.days).map(([day, dayData]) => (
                <tr key={day} className="border-b border-gray-100">
                  <td className="px-4 py-3 font-semibold text-gray-700">{day}</td>
                  <td className="px-4 py-3 text-center text-gray-600">{dayData.count}</td>
                  <td className="px-4 py-3 text-center text-gray-600">
                    {minutesToTime(dayData.totalWait / dayData.count)}
                  </td>
                  <td className="px-4 py-3 text-center text-gray-600">
                    {minutesToTime(dayData.maxWait)}
                  </td>
                  <td className="px-4 py-3 text-center text-gray-600">
                    {minutesToTime(dayData.minWait)}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#cb681c10] p-6 rounded-xl border border-[#cb691c]">
              ...
              <p>λ: {statistics.lambda.toFixed(3)} clientes/min</p>
              <p>μ: {statistics.mu.toFixed(3)} clientes/min</p>
            ...
            <div className="bg-[#fbd9160f] p-6 rounded-xl border border-[#fbd916]">
              ...
              <p>λ: {simulationStats.lambda.toFixed(3)} clientes/min</p>
              <p>μ: {simulationStats.mu.toFixed(3)} clientes/min</p>
